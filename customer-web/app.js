...
async function loginEmail(){
  ...
  try{
    if(SIGNUP){
      ...
      const d={name,email,phone:mobile,photoUrl:"",role:"CUSTOMER",address:"",createdAt:FV.serverTimestamp()};
      await db.collection("users").doc(c.user.uid).set(d);
      ME={uid:c.user.uid,...d}; startApp();
    } else {
      await auth.signInWithEmailAndPassword(email,pass);   // onAuthStateChanged → handleUser
    }
  }catch(e){ BUSY=false; loginErr(e.message); }
}
// Central router: existing user → dashboard; brand-new Google user → profile setup form.
...
// True if the signed-in user has an email but no email/password credential yet.
...
async function completeGoogleSignup(){
  ...
  const name=($("ps-name").value||"").trim(), mobile=($("ps-mobile").value||"").trim(), pass=$("ps-pass").value||"";
  if(!name) return loginErr("Please enter your name.");
  const btn=$("ps-save"); if(btn){ btn.disabled=true; btn.textContent="Creating…"; }
  try{
    if(pass && pass.length>=6 && u.email){
      try{ await u.linkWithCredential(firebase.auth.EmailAuthProvider.credential(u.email,pass)); }catch(e){ /* already linked / in use — fine */ }
    }
    const d={name,email:u.email||"",phone:mobile,photoUrl:u.photoURL||"",role:"CUSTOMER",address:"",createdAt:FV.serverTimestamp()};
